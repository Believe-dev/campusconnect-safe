...
import { useProfileMenuItems, type ProfileMenuItem } from "@/components/layout/useProfileMenuItems";
...
const gridClass = "grid grid-cols-3 gap-3 sm:grid-cols-4 lg:grid-cols-5";
...
export const ProfileSheet = ({ open, onOpenChange }: ProfileSheetProps) => {
  const { user, profile, primary, secondary, handleSignOut, SignOutIcon } = useProfileMenuItems();

  if (!user) return null;

  const renderItem = ({ key, to, label, icon: Icon, badge, onClick }: ProfileMenuItem) => (
    <Link
      key={key}
      to={to}
      onClick={() => {
      ...
      className="relative flex flex-col items-center gap-2 rounded-2xl bg-white p-4 text-center shadow-card transition hover:brightness-105 active:scale-95"
    >
      <span className="flex h-11 w-11 items-center justify-center rounded-full bg-flora-chip text-flora-leaf">
        <Icon className="h-5 w-5" />
      </span>
      <span className="text-sm font-medium text-flora-ink">{label}</span>
      {!!badge && badge > 0 && (
        ...
        >
          <div className="mx-auto w-full max-w-3xl px-5">
            <div className="flex items-center justify-between">
              <h1 className="text-xl font-semibold text-flora-ink">Profile</h1>
              <button
                type="button"
                aria-label="Close"
            ...
            >
              ...
              <div className="flex min-w-0 flex-col">
                <p className="truncate font-semibold text-flora-ink">{profile?.full_name}</p>
                <p className="truncate text-sm text-flora-muted">{user.email}</p>
            ...
            <div className={`mt-5 ${gridClass}`}>{primary.map(renderItem)}</div>

            <div className="my-5 h-px bg-flora-ink/10" />

            <div className={gridClass}>{secondary.map(renderItem)}</div>
            ...
            <div className="mt-5">
              <button
                ...
                className="flex w-full items-center gap-4 rounded-2xl bg-white px-4 py-3.5 text-base text-destructive shadow-card transition-colors hover:bg-destructive/5"
              >
