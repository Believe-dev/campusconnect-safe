...
export const SellerSubscriptionPayment = ({ onSuccess, onCancel }: SellerSubscriptionPaymentProps) => {
  ...
  const handlePayment = async () => {
    ...
    try {
      // Initialize Anchor BaaS subscription payment
      const paymentRef = `ANCHOR_SUB_${user.id}_${Date.now()}`;
      toast.success('Processing payment via Anchor BaaS...');
      
      const success = await createSubscription(paymentRef);
      if (success) {
        toast.success('Subscription activated via Anchor BaaS! You now have full seller access.');
        onSuccess?.();
      } else {
