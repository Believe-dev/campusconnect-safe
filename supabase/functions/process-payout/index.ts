...
serve(async (req) => {
  ...
  try {
    ...
    const anchorApiKey = Deno.env.get('VITE_ANCHOR_API_KEY') || Deno.env.get('ANCHOR_API_KEY')

    if (!supabaseUrl || !supabaseKey) {
      throw new Error('Missing required Supabase environment variables')
    }

    console.log('Processing Anchor BaaS payout request:', payoutId)
    console.log('Anchor API key configured:', anchorApiKey ? 'Yes' : 'No')
    ...
    // Generate Anchor BaaS payout transfer code
    transferCode = `ANCHOR_TRF_${Date.now()}_${payoutId.slice(0, 8)}`
    isRealTransfer = true
    console.log('Anchor BaaS transfer code generated:', transferCode)
